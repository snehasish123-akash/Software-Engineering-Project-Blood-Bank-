...
try {
    ...
    $stmt = $pdo->query("SELECT id, full_name, blood_group, gender, phone, address FROM users WHERE user_type = 'donor' AND status = 'active' ORDER BY full_name");
    $donors = $stmt->fetchAll();
    
    // Get blood group counts for filter
    $bloodGroupCounts = [];
    ...
    error_log("Error in donor-list.php: " . $e->getMessage());
    $donors = [];
    $bloodGroupCounts = [];
}
...
<body>
            ...
            <div class="row">
                <div class="col-12 text-center">
                    <h1 class="display-4 fw-bold text-primary mb-3">
                        <i class="fas fa-users me-3"></i>Blood Donors
                    </h1>
                    <p class="lead text-muted">Connect with our registered blood donors and save lives</p>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-2">
                    <div class="text-muted">
                        <small><?php echo count($donors); ?> donors found</small>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Donors Grid -->
    <section class="py-5">
        <div class="container donor-list-container">
            <div class="row g-4" id="donorsGrid">
                <?php if (empty($donors)): ?>
                    <div class="col-12">
                        <div class="text-center py-5">
                            <i class="fas fa-users text-muted" style="font-size: 4rem;"></i>
                            <h4 class="mt-3 text-muted">No donors found</h4>
                            <p class="text-muted">Be the first to register as a blood donor!</p>
                            <a href="register.php?type=donor" class="btn btn-primary">
                    ...
                    <?php foreach ($donors as $donor): ?>
                        <div class="col-lg-4 col-md-6">
                            <div class="donor-card" 
                                 data-blood-group="<?php echo htmlspecialchars($donor['blood_group']); ?>"
                                 data-name="<?php echo strtolower($donor['full_name']); ?>"
                                 data-location="<?php echo strtolower($donor['address']); ?>">
